  ...
  reader.onload = function (e) {
    ...
    contenido.forEach(item => {
      ...
      if (!idTropa) {
        ...
        idTropa = tropaObj ? tropaObj.IDtropa : null;
      }
      fila.dataset.idTropa = idTropa;
      ...
      fila.addEventListener("click", function () {
        document.querySelectorAll("#tablaUnidades tr").forEach(f => f.classList.remove("selected"));
        fila.classList.add("selected");

        if (idTropa) {
          mostrarFicha(idTropa);
          mostrarReglas(idTropa);
          mostrarImagen(idTropa);
        }
      });
